...
class _KioskDashboardState extends State<KioskDashboard> {
  ...
  // Variables to hold guest data for equipment reservation
  ...
  void _handleDeptClick(String dept) {
    if (widget.isGuest) {
      _showGuestForm(
        ...
        onContinue: (name, id, phone, email) {
          Navigator.push(context, MaterialPageRoute(builder: (p) => AppointmentPage(department: dept, userName: name, userId: id, isGuest: true, guestPhone: phone, guestEmail: email)));
        }
      );
    } else {
                            ...
                            actions: [
                              ...
                              ElevatedButton(
                                onPressed: () async {
                                  ...
                                  // THE FIX: Storing the guest's phone and email in the database
...
class _AppointmentPageState extends State<AppointmentPage> {
            ...
            child: selDate == null
                ...
                : GridView.count(
                    ...
                    children: _generateTimeSlots(selDate).map((time) {
                      ...
                      bool isBooked = booked.contains(time);
                      bool isPassed = _isPastTime(time);
                      
                      Color bgColor = Colors.white;
                      Color fgColor = Colors.black87;
                      String buttonText = time;
                      ...
                      return ElevatedButton(
                        style: ElevatedButton.styleFrom(
                          ...
                          disabledBackgroundColor: isBooked ? Colors.red.shade50 : Colors.grey.shade100,
                          disabledForegroundColor: isBooked ? Colors.red : Colors.grey.shade400,
                        ),
                        onPressed: (isBooked || isPassed) ? null : () => setState(() => selTime = time),
  ...
  void _submit() async {
    ...
    if (widget.isGuest) { 
      data['phone'] = widget.guestPhone; 
      data['email'] = widget.guestEmail; 
    }
    
    await FirebaseFirestore.instance.collection('appointments').add(data);

    // --- TRIGGER EMAILJS FOR APPOINTMENTS ---
    String? recipientEmail = widget.isGuest ? widget.guestEmail : "s${widget.userId}@studentmail.unimap.edu.my";
