...
class _KioskDashboardState extends State<KioskDashboard> {
  ...
  void _autoLogOut() {
    ...
    Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (c) => const KioskLoginPage()), (r) => false);
  }
  ...
  Widget _buildStatusItem(String label, String value, Color valColor) {
    return Column(
      children: [
        ...
        Text(value, style: TextStyle(fontSize: 42, fontWeight: FontWeight.bold, color: valColor)),
      ],
    );
  }
  ...
  void _handleDeptClick(String dept) async {
    if (widget.isGuest) {
      _showGuestForm(
        ...
        onContinue: (name, id, phone, email) async {
          String? result = await Navigator.push(context, MaterialPageRoute(builder: (p) => AppointmentPage(
    ...
    return Column(
      ...
      children: [
        ...
        const Padding(
          ...
          child: Text("Equipment Loan Request", style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Color(0xFF133F85)))
        ),
        Expanded(
